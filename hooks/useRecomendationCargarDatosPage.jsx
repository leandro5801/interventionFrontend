...
export default function useRecomendationsCargarDatosPage() {
  ...
  if (user && consultores) {
    ...
    if (consultorAutenticado) {
      if (user.id_rol === 3) {
        filtredProjects = projects.filter((project) => project.cargar_proyecto);
        filtredInterventions = interventions.filter((i) =>
          filtredProjects.some(
            (project) => project.id_proyecto === i.id_proyecto
          )
        );
        filtredRecomendations = recomendations.filter((recomendation) =>
          filtredInterventions.some(
            (intervention) =>
              intervention.id_intervencion === recomendation.id_intervencion
          )
        );
      }
    }
